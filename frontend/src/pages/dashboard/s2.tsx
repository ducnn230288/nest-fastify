  ...
  function generateFields(updatedResult: any) {
    return {
      ...
      values: updatedResult.values.items
        .filter((i: any) => !updatedResult.values.hideItems.find((hide: any) => hide.id === i.id))
        .map((i: any) => i.id),
      valueInCols: true,
    };
  }
  const [fields, setFields] = useState(s2DataConfig.fields);
  const [switcherFields, setSwitcherFields] = useState(defaultSwitcherFields);
