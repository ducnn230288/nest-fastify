...
export const EChart = forwardRef(({ option }: Type, ref: Ref<any>) => {
  ...
  if (option) {
    ...
    let yAxis: YAXisOption | YAXisOption[] | undefined = { splitLine: { lineStyle: { type: 'dashed' } }, scale: true };
    switch (option.type) {
      case ETypeChart.bar:
    ...
    optionsEcharts = { title, xAxis, yAxis, series, tooltip, legend };
  }
  ...
  //   console.log(param, echarts);
  // };
  return optionsEcharts ? (
    <ReactEChartsCore
      echarts={echarts}
      option={optionsEcharts}
      notMerge={true}
      lazyUpdate={true}
