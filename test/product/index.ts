...
import { Category, Product, StoreRequest } from '@model';
import { CreateCategoryRequestDto, CreateCodeRequestDto, CreateProductRequestDto, CreateStoreRequestDto, DetailCategoryResponeDto, RejectStoreRequestDto } from '@dto';
import '@factories';
...
export const testCase = (type?: string, permissions: string[] = []): void => {
  ...
  it(`Create [POST ${API}]`, async () => {
    dataCreate = await factoryManager.get(Product).make();
    ...
    const { body } = await request(BaseTest.server)
      ...
      .send(dataCreate as CreateProductRequestDto)
