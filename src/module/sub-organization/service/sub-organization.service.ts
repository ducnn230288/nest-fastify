...
//   }
  async add(body: CreateSubOrganizationRequestDto, user: User): Promise<any> {
    ...
    if (await this.repo.checkFax(user, body)) throw new ConflictException(i18n?.t('SubOrg.Fax number has been previously registered'));
  ...
  async updateSubOrganization(subOrgDto: UpdateSubOrganizationDto, id: string, user: User) {
    const i18n = I18nContext.current();
    const {
        emailContact,
        phoneNumber,
    ...
    if (!isNullOrUndefined(checkFax)) throw new ConflictException(i18n?.t('SubOrg.Fax number has been previously registered'));
